...
NSString *NameOfEndpoint(MIDIEndpointRef ref)
{
    NSString *string = nil;
    OSStatus s = noErr;
    s = MIDIObjectGetStringProperty(ref, (CFStringRef)@"name", (CFStringRef *)&string);
    if (noErr == s) {
        return string;
    }
    ...
    s = MIDIObjectGetStringProperty(entity, (CFStringRef)@"name", (CFStringRef *)&string);
    if (s)
    {
        string = @"Unknown name";
    }
...
@synthesize isEnabledForSending;
...
- (id) init
{
    if ((self = [super init]))
    {
        ...
        OSStatus s = MIDIClientCreate((CFStringRef)@"Brian's Awesome Synth Guitar", PGMIDINotifyProc, arc_cast<void>(self), &client);
        NSLogError(s, @"Create MIDI client");

        s = MIDIOutputPortCreate(client, (CFStringRef)@"Brian's Awesome Synth Guitar", &outputPort);
        NSLogError(s, @"Create output MIDI port");

        s = MIDIInputPortCreate(client, (CFStringRef)@"Brian's Awesome Synth Guitar", PGMIDIReadProc, arc_cast<void>(self), &inputPort);
...
- (void) sendPacketList:(const MIDIPacketList *)packetList
{
    for (ItemCount index = 0; index < MIDIGetNumberOfDestinations(); ++index)
    {
        ...
        if (outputEndpoint)
        {
            PGMidiDestination *destination = [self getDestination:outputEndpoint];
            if (destination) {
                if (NO == destination.isEnabledForSending) {
                    continue;
                }
            }
            // Send it
